...
from app.services.byo import cfg as _cfg  # BYO: prefers per-request keys, falls back to config.app
...
def _generate_response(prompt: str) -> str:
    try:
        ...
        if llm_provider == "g4f":
            ...
            model_name = _cfg("g4f_model_name", "")
            if not model_name:
            ...
            if llm_provider == "moonshot":
                api_key = _cfg("moonshot_api_key")
                model_name = _cfg("moonshot_model_name")
                base_url = "https://api.moonshot.cn/v1"
            elif llm_provider == "ollama":
                # api_key = _cfg("openai_api_key")
                api_key = "ollama"  # any string works but you are required to have one
                model_name = _cfg("ollama_model_name")
                base_url = _cfg("ollama_base_url", "")
                if not base_url:
                    base_url = config.get_default_ollama_base_url()
            elif llm_provider == "openai":
                api_key = _cfg("openai_api_key")
                model_name = _cfg("openai_model_name")
                base_url = _cfg("openai_base_url", "")
                if not base_url:
                    base_url = "https://api.openai.com/v1"
            elif llm_provider == "oneapi":
                api_key = _cfg("oneapi_api_key")
                model_name = _cfg("oneapi_model_name")
                base_url = _cfg("oneapi_base_url", "")
            elif llm_provider == "azure":
                api_key = _cfg("azure_api_key")
                model_name = _cfg("azure_model_name")
                base_url = _cfg("azure_base_url", "")
                api_version = _cfg("azure_api_version", "2024-02-15-preview")
            elif llm_provider == "gemini":
                api_key = _cfg("gemini_api_key")
                model_name = _cfg("gemini_model_name")
                ...
                api_key = _cfg("grok_api_key")
                model_name = _cfg("grok_model_name")
                base_url = _cfg("grok_base_url", "")
                if not base_url:
                    base_url = "https://api.x.ai/v1"
            elif llm_provider == "qwen":
                api_key = _cfg("qwen_api_key")
                model_name = _cfg("qwen_model_name")
                base_url = "***"
            elif llm_provider == "cloudflare":
                api_key = _cfg("cloudflare_api_key")
                model_name = _cfg("cloudflare_model_name")
                account_id = _cfg("cloudflare_account_id")
                base_url = "***"
            elif llm_provider == "minimax":
                api_key = _cfg("minimax_api_key")
                model_name = _cfg("minimax_model_name")
                base_url = _cfg("minimax_base_url", "")
                if not base_url:
                    base_url = "https://api.minimax.io/v1"
            elif llm_provider == "mimo":
                api_key = _cfg("mimo_api_key")
                model_name = _cfg("mimo_model_name")
                base_url = _cfg("mimo_base_url", "")
                ...
                model_name = _cfg("deepseek_model_name")
                base_url = _cfg("deepseek_base_url")
                if not base_url:
                    base_url = "https://api.deepseek.com"
            elif llm_provider == "modelscope":
                api_key = _cfg("modelscope_api_key")
                model_name = _cfg("modelscope_model_name")
                base_url = _cfg("modelscope_base_url")
                if not base_url:
                    base_url = "https://api-inference.modelscope.cn/v1/"
            elif llm_provider == "ernie":
                api_key = _cfg("ernie_api_key")
                secret_key = _cfg("ernie_secret_key")
                base_url = _cfg("ernie_base_url")
                model_name = "***"
                if not secret_key:
                    raise ValueError(
                ...
                try:
                    base_url = _cfg("pollinations_base_url", "")
                    if not base_url:
                        base_url = "https://text.pollinations.ai/openai"
                    model_name = _cfg("pollinations_model_name", "openai-fast")
                    ...
                    if _cfg("pollinations_private"):
                        payload["private"] = True
                    if _cfg("pollinations_referrer"):
                        payload["referrer"] = _cfg("pollinations_referrer")
                ...
                model_name = _cfg("litellm_model_name")
