...
# Shared MM->MPT bearer token (x-api-key). Set via env on PaaS hosts
# (Render/Railway) instead of mounting a config.toml. When non-empty, the API
# router enforces it so only the orchestrator (Marketing Magic) can call us.
app["api_key"] = os.getenv("MPT_API_KEY", app.get("api_key", ""))
